...
template< typename MatrixType >
bool
warpList< MatrixType >::addWarp( const IndexType offset,
...
template< typename Real, typename Device, typename Index >
void
AdEllpack< Real, Device, Index >::setCompressedRowLengths( ConstRowCapacitiesTypeView rowLengths )
{
...
template< typename Real, typename Device, typename Index >
void
AdEllpack< Real, Device, Index >::setRowCapacities( ConstRowCapacitiesTypeView rowLengths )
{
...
template< typename Real, typename Device, typename Index >
void
AdEllpack< Real, Device, Index >::getCompressedRowLengths( RowCapacitiesTypeView rowLengths ) const
{
...
template< typename Real, typename Device, typename Index >
Index
AdEllpack< Real, Device, Index >::getTotalLoad() const
{
...
template< typename Real, typename Device, typename Index >
void
AdEllpack< Real, Device, Index >::performRowLengthsTest( ConstRowCapacitiesTypeView rowLengths )
{
   bool found = false;
   for( IndexType row = 0; row < this->getRows(); row++ ) {
      found = false;
      ...
      while( ! found ) {
         ...
         else if( ( inWarpOffset == this->warpSize - 1 ) && ( this->rowOffset.getElement( warp + 1 ) == row ) ) {
            warp++;
            inWarpOffset = 0;
...
template< typename Real, typename Device, typename Index >
void
AdEllpack< Real, Device, Index >::performRowTest()
{
...
template< typename Real, typename Device, typename Index >
Index
AdEllpack< Real, Device, Index >::getWarp( const IndexType row ) const
{
...
template< typename Real, typename Device, typename Index >
Index
AdEllpack< Real, Device, Index >::getInWarpOffset( const IndexType row, const IndexType warp ) const
{
...
template< typename Real, typename Device, typename Index >
Index
AdEllpack< Real, Device, Index >::getRowLength( const IndexType row ) const
{
   IndexType warp = this->getWarp( row );
   IndexType inWarpOffset = this->getInWarpOffset( row, warp );
   if( inWarpOffset == 0 && rowOffset.getElement( warp ) != row )
...
void
AdEllpack< Real, Device, Index >::setLike( const AdEllpack< Real2, Device2, Index2 >& matrix )
{
...
template< typename Real, typename Device, typename Index >
void
AdEllpack< Real, Device, Index >::reset()
{
...
template< typename Real2, typename Device2, typename Index2 >
bool
AdEllpack< Real, Device, Index >::operator==( const AdEllpack< Real2, Device2, Index2 >& matrix ) const
{
...
template< typename Real, typename Device, typename Index >
bool
AdEllpack< Real, Device, Index >::setElement( const IndexType row, const IndexType column, const RealType& value )
{
...
template< typename Real, typename Device, typename Index >
bool
AdEllpack< Real, Device, Index >::addElement( const IndexType row,
...
template< typename Real, typename Device, typename Index >
bool
AdEllpack< Real, Device, Index >::setRow( const IndexType row,
...
template< typename Real,
          ...
          typename Index >
bool
AdEllpack< Real, Device, Index >::addRow( const IndexType row,
                                          const IndexType* columnIndexes,
                                          const RealType* values,
                                          const IndexType elements,
                                          const RealType& thisElementMultiplicator )
...
template< typename Real, typename Device, typename Index >
Real
AdEllpack< Real, Device, Index >::getElement( const IndexType row, const IndexType column ) const
{
   IndexType warp = this->getWarp( row );
   IndexType inWarpOffset = this->getInWarpOffset( row, warp );
   if( inWarpOffset == 0 && rowOffset.getElement( warp ) != row )
...
template< typename Real, typename Device, typename Index >
void
AdEllpack< Real, Device, Index >::getRow( const IndexType row, IndexType* columns, RealType* values ) const
{
   IndexType warp = this->getWarp( row );
   IndexType inWarpOffset = this->getInWarpOffset( row, warp );
   if( inWarpOffset == 0 && rowOffset.getElement( warp ) != row )
...
void
AdEllpack< Real, Device, Index >::vectorProduct( const InVector& inVector, OutVector& outVector ) const
{
...
template< typename Real, typename Device, typename Index >
void
AdEllpack< Real, Device, Index >::save( File& file ) const
{
...
template< typename Real, typename Device, typename Index >
void
AdEllpack< Real, Device, Index >::load( File& file )
{
...
template< typename Real, typename Device, typename Index >
void
AdEllpack< Real, Device, Index >::save( const String& fileName ) const
{}

template< typename Real, typename Device, typename Index >
void
AdEllpack< Real, Device, Index >::load( const String& fileName )
{}

template< typename Real, typename Device, typename Index >
void
AdEllpack< Real, Device, Index >::print( std::ostream& str ) const
{
   for( IndexType row = 0; row < this->getRows(); row++ ) {
      str << "Row: " << row << " -> ";
      ...
      while( ! found ) {
         ...
         for( IndexType i = 0; i < this->localLoad.getElement( warp ); i++ ) {
            if( this->columnIndexes.getElement( elementPtr ) != this->getPaddingIndex() )
               str << " Col:" << this->columnIndexes.getElement( elementPtr ) << "->" << this->values.getElement( elementPtr )
                   << "\t";
            elementPtr += this->warpSize;
         }
         if( ( inWarpOffset < this->warpSize - 1 )
...
template< typename Real, typename Device, typename Index >
bool
AdEllpack< Real, Device, Index >::balanceLoad( const RealType average,
...
template< typename Real, typename Device, typename Index >
void
AdEllpack< Real, Device, Index >::computeWarps( const IndexType SMs, const IndexType threadsPerSM, warpList< AdEllpack >* list )
{
...
template< typename Real, typename Device, typename Index >
bool
AdEllpack< Real, Device, Index >::createArrays( warpList< AdEllpack >* list )
{
   ...
   this->offset.setElement( length,
                            this->offset.getElement( length - 1 ) + this->warpSize * this->localLoad.getElement( length - 1 ) );
...
public:
   ...
   static void
   vectorProduct( const AdEllpack< Real, Device, Index >& matrix, const InVector& inVector, OutVector& outVector )
   {
...
__device__
void
AdEllpack< Real, Device, Index >::spmvCuda2( const InVector& inVector, OutVector& outVector, const int gridIdx ) const
{
   ...
   if( ( inWarpIdx == 0 ) || ( reduceMap[ threadIdx.x ] > reduceMap[ threadIdx.x - 1 ] ) ) {
      ...
      globalIdx++;
      while( globalIdx < ( ( warpIdx + 1 ) << 5 ) && reduceMap[ elementPtr ] == reduceMap[ threadIdx.x ] ) {
...
__device__
void
AdEllpack< Real, Device, Index >::spmvCuda4( const InVector& inVector, OutVector& outVector, const int gridIdx ) const
{
   IndexType globalIdx = ( gridIdx * Backend::getMaxGridXSize() + blockIdx.x ) * blockDim.x + threadIdx.x;
   IndexType warpIdx = globalIdx >> 5;
   IndexType inWarpIdx = globalIdx & ( this->warpSize - 1 );
...
__device__
void
AdEllpack< Real, Device, Index >::spmvCuda8( const InVector& inVector, OutVector& outVector, const int gridIdx ) const
{
   IndexType globalIdx = ( gridIdx * Backend::getMaxGridXSize() + blockIdx.x ) * blockDim.x + threadIdx.x;
   IndexType warpIdx = globalIdx >> 5;
   IndexType inWarpIdx = globalIdx & ( this->warpSize - 1 );
   ...
   if( warpLoad < 4 ) {
      while( i < warpLoad && this->columnIndexes[ elementPtr ] < this->getColumns() ) {
         temp[ threadIdx.x ] += inVector[ this->columnIndexes[ elementPtr ] ] * this->values[ elementPtr ];
         elementPtr += this->warpSize;
         i++;
      }
      ...
      while( alignUnroll != 0 && this->columnIndexes[ elementPtr ] < this->getColumns() ) {
         temp[ threadIdx.x ] += inVector[ this->columnIndexes[ elementPtr ] ] * this->values[ elementPtr ];
         elementPtr += this->warpSize;
         i++;
         alignUnroll--;
...
__device__
void
AdEllpack< Real, Device, Index >::spmvCuda16( const InVector& inVector, OutVector& outVector, const int gridIdx ) const
{
   IndexType globalIdx = ( gridIdx * Backend::getMaxGridXSize() + blockIdx.x ) * blockDim.x + threadIdx.x;
   IndexType warpIdx = globalIdx >> 5;
   IndexType inWarpIdx = globalIdx & ( this->warpSize - 1 );
...
__device__
void
AdEllpack< Real, Device, Index >::spmvCuda32( const InVector& inVector, OutVector& outVector, const int gridIdx ) const
{
   IndexType globalIdx = ( gridIdx * Backend::getMaxGridXSize() + blockIdx.x ) * blockDim.x + threadIdx.x;
   IndexType warpIdx = globalIdx >> 5;
   IndexType inWarpIdx = globalIdx & ( this->warpSize - 1 );
...
__global__
void
AdEllpackVectorProductCuda16( const AdEllpack< Real, Devices::Cuda, Index >* matrix,
                              const InVector* inVector,
                              OutVector* outVector,
                              const int gridIdx )
{
...
__global__
void
AdEllpackVectorProductCuda32( const AdEllpack< Real, Devices::Cuda, Index >* matrix,
                              const InVector* inVector,
                              OutVector* outVector,
                              const int gridIdx )
{
