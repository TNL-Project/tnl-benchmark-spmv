 ...
 */
template <typename MatrixType> warpList<MatrixType>::warpList() {
...
AdEllpack<Real, Device, Index>::AdEllpack() : warpSize(32) {}

template <typename Real, typename Device, typename Index>
void AdEllpack<Real, Device, Index>::setCompressedRowLengths(
  ...
  if (std::is_same<DeviceType, Devices::Cuda>::value) {
    ...
    this->allocateMatrixElements(
        this->offset.getElement(this->offset.getSize() - 1));
  }
}

template <typename Real, typename Device, typename Index>
void AdEllpack<Real, Device, Index>::setRowCapacities(
    ConstRowCapacitiesTypeView rowLengths) {
  setCompressedRowLengths(rowLengths);
}

template <typename Real, typename Device, typename Index>
void AdEllpack<Real, Device, Index>::getCompressedRowLengths(
...
template <typename Real, typename Device, typename Index>
void AdEllpack<Real, Device, Index>::performRowLengthsTest(
    ConstRowCapacitiesTypeView rowLengths) {
  bool found = false;
  for (IndexType row = 0; row < this->getRows(); row++) {
    found = false;
    ...
    while (!found) {
      if ((inWarpOffset < this->warpSize - 1) &&
          (this->reduceMap.getElement(this->warpSize * warp + inWarpOffset +
                                      1) == row)) {
        inWarpOffset++;
...
void AdEllpack<Real, Device, Index>::performRowTest() {
  for (IndexType warp = 0; warp < this->localLoad.getSize(); warp++) {
    ...
    for (IndexType i = warp * this->warpSize + 1;
         i < (warp + 1) * this->warpSize; i++) {
      if (this->reduceMap.getElement(i) == 1)
        row++;
    }
    if (row == this->rowOffset.getElement(warp + 1) ||
        row + 1 == this->rowOffset.getElement(warp + 1))
      ;
    else {
      std::cout << "Error warp = " << warp << std::endl;
      std::cout << "Row: " << row
                << ", Row offset: " << this->rowOffset.getElement(warp + 1)
                << std::endl;
    }
  }
}
...
Index AdEllpack<Real, Device, Index>::getWarp(const IndexType row) const {
  for (IndexType searchedWarp = 0; searchedWarp < this->getRows();
       searchedWarp++) {
    if ((this->rowOffset.getElement(searchedWarp) == row) ||
        ((this->rowOffset.getElement(searchedWarp) < row) &&
         (this->rowOffset.getElement(searchedWarp + 1) >= row)))
      return searchedWarp;
  }
...
template <typename Real, typename Device, typename Index>
Index AdEllpack<Real, Device, Index>::getInWarpOffset(
    const IndexType row, const IndexType warp) const {
  IndexType inWarpOffset = warp * this->warpSize;
  while ((inWarpOffset < (warp + 1) * this->warpSize) &&
         (this->reduceMap.getElement(inWarpOffset) < row))
    inWarpOffset++;
...
Index AdEllpack<Real, Device, Index>::getRowLength(const IndexType row) const {
  ...
  while (!found) {
    ...
    if ((inWarpOffset < this->warpSize - 1) &&
        (this->reduceMap.getElement(this->warpSize * warp + inWarpOffset + 1) ==
         row))
      inWarpOffset++;
    else if ((inWarpOffset == this->warpSize - 1) &&
             (this->rowOffset.getElement(warp + 1) == row)) {
...
void AdEllpack<Real, Device, Index>::setLike(
    const AdEllpack<Real2, Device2, Index2> &matrix) {
...
template <typename Real, typename Device, typename Index>
template <typename Real2, typename Device2, typename Index2>
bool AdEllpack<Real, Device, Index>::operator==(
    const AdEllpack<Real2, Device2, Index2> &matrix) const {
...
template <typename Real, typename Device, typename Index>
bool AdEllpack<Real, Device, Index>::addElement(
  ...
  while (!found) {
    ...
    for (IndexType i = 0; i < this->localLoad.getElement(warp); i++) {
      ...
      if (this->columnIndexes.getElement(elementPtr) == column) {
        this->values.setElement(elementPtr,
...
bool AdEllpack<Real, Device, Index>::setRow(const IndexType row,
  ...
  while ((!found) && (elPtr < elements)) {
    elementPtr = this->offset.getElement(warp) + inWarpOffset;
    for (IndexType i = 0;
         (i < this->localLoad.getElement(warp)) && (elPtr < elements); i++) {
...
template <typename Real, typename Device,
          ...
          typename Index>
bool AdEllpack<Real, Device, Index>::addRow(
    const IndexType row, const IndexType *columnIndexes, const RealType *values,
    const IndexType elements, const RealType &thisElementMultiplicator) {
  return false;
}
...
AdEllpack<Real, Device, Index> &AdEllpack<Real, Device, Index>::operator=(
    const AdEllpack<Real2, Device2, Index2> &matrix) {
...
template <typename Real, typename Device, typename Index>
bool AdEllpack<Real, Device, Index>::balanceLoad(
    const RealType average, ConstRowCapacitiesTypeView rowLengths,
  ...
  for (IndexType row = 0; row < this->getRows(); row++) {
    ...
    IndexType threadsPerRow =
        (IndexType)(rowLength / ave) + (rowLength % ave == 0 ? 0 : 1);
    // if this number added to number of total threads in this warp is higher
    // than 32 then spreads them in as many as necessary
    ...
    while (numberOfThreads + threadsPerRow >= this->warpSize) {
      ...
      // if it doesnt apply and if local load isn't equal 0 it will use old
      // local load ( for better balance )
      else if (localLoad != 0) {
    ...
    localLoad = max(localLoad, roundUpDivision(rowLength, threadsPerRow));
    reduceMap[numberOfThreads] = 1;
    for (IndexType i = numberOfThreads + 1; i < numberOfThreads + threadsPerRow;
         i++)
      reduceMap[i] = 0;

    numberOfThreads += threadsPerRow;

    // if last row doesnt fill the whole warp or it fills more warps and threads
    // still remain
    if (((row == this->getRows() - 1) && !addedWarp) ||
        ((row == this->getRows() - 1) && (threadsPerRow == numberOfThreads) &&
         (numberOfThreads > 0))) {
...
bool AdEllpack<Real, Device, Index>::createArrays(warpList<AdEllpack> *list) {
  ...
  while (warp != list->getTail()) {
    ...
    for (int i = iteration * this->warpSize + 1;
         i < (iteration + 1) * this->warpSize; i++) {
  ...
  this->offset.setElement(
      length, this->offset.getElement(length - 1) +
                  this->warpSize * this->localLoad.getElement(length - 1));
  this->allocateMatrixElements(this->offset.getElement(length));

  return true;
}

template <> class AdEllpackDeviceDependentCode<Devices::Host> {
public:
  ...
  static void vectorProduct(const AdEllpack<Real, Device, Index> &matrix,
    ...
    const Index blocks = (Index)(matrix.reduceMap.getSize() / blockSize) +
                         (matrix.reduceMap.getSize() % blockSize != 0);
    for (Index block = 0; block < blocks; block++) {
      ...
      for (Index threadIdx = 0; threadIdx < blockSize; threadIdx++) {
        ...
        for (Index i = 0; i < matrix.localLoad.getElement(warpIdx); i++) {
          if (matrix.columnIndexes.getElement(elementPtr) !=
              matrix.getPaddingIndex())
            temp.setElement(
                threadIdx,
                temp.getElement(threadIdx) +
                    matrix.values.getElement(elementPtr) *
                        inVector[matrix.columnIndexes.getElement(elementPtr)]);
      ...
      for (Index threadIdx = 0; threadIdx < blockSize; threadIdx++) {
        ...
        if (globalIdx >= matrix.reduceMap.getSize())
          break;
        if ((inWarpIdx == 0) || (reduceMap.getElement(threadIdx - 1) <
                                 reduceMap.getElement(threadIdx))) {
          Index end = (warpIdx + 1) << 5;
          Index elementPtr = threadIdx + 1;
          globalIdx++;
          while (globalIdx < end && reduceMap.getElement(elementPtr) ==
                                        reduceMap.getElement(threadIdx)) {
            temp.setElement(threadIdx, temp.getElement(elementPtr) +
                                           temp.getElement(threadIdx));
            elementPtr++;
            globalIdx++;
          }
          outVector[reduceMap.getElement(threadIdx)] +=
              temp.getElement(threadIdx);
        }
      }
    }
...
template <typename Real, typename Device, typename Index>
template <typename InVector, typename OutVector>
__device__ void AdEllpack<Real, Device, Index>::spmvCuda2(
    const InVector &inVector, OutVector &outVector, const int gridIdx) const {
  IndexType globalIdx =
  ...
  for (; i < warpLoad; i++) {
    if (this->columnIndexes[elementPtr] < this->getColumns()) {
      temp[threadIdx.x] +=
          inVector[this->columnIndexes[elementPtr]] * this->values[elementPtr];
      elementPtr += this->warpSize;
    }
  }

  if ((inWarpIdx == 0) ||
      (reduceMap[threadIdx.x] > reduceMap[threadIdx.x - 1])) {
...
template <typename Real, typename Device, typename Index>
template <typename InVector, typename OutVector>
__device__ void AdEllpack<Real, Device, Index>::spmvCuda4(
    const InVector &inVector, OutVector &outVector, const int gridIdx) const {
  IndexType globalIdx =
      (gridIdx * Backend::getMaxGridXSize() + blockIdx.x) * blockDim.x +
      threadIdx.x;
...
#pragma unroll
  ...
  if ((inWarpIdx == 0) ||
    ...
    IndexType elementPtr = threadIdx.x + 1;
    globalIdx++;
    while (globalIdx < end && reduceMap[elementPtr] == reduceMap[threadIdx.x]) {
      temp[threadIdx.x] += temp[elementPtr];
      elementPtr++;
      globalIdx++;
    }
...
template <typename Real, typename Device, typename Index>
template <typename InVector, typename OutVector>
__device__ void AdEllpack<Real, Device, Index>::spmvCuda8(
    const InVector &inVector, OutVector &outVector, const int gridIdx) const {
  IndexType globalIdx =
      (gridIdx * Backend::getMaxGridXSize() + blockIdx.x) * blockDim.x +
      threadIdx.x;
...
template <typename Real, typename Device, typename Index>
template <typename InVector, typename OutVector>
__device__ void AdEllpack<Real, Device, Index>::spmvCuda16(
    const InVector &inVector, OutVector &outVector, const int gridIdx) const {
  IndexType globalIdx =
      (gridIdx * Backend::getMaxGridXSize() + blockIdx.x) * blockDim.x +
      threadIdx.x;
...
template <typename Real, typename Device, typename Index>
template <typename InVector, typename OutVector>
__device__ void AdEllpack<Real, Device, Index>::spmvCuda32(
    const InVector &inVector, OutVector &outVector, const int gridIdx) const {
  IndexType globalIdx =
      (gridIdx * Backend::getMaxGridXSize() + blockIdx.x) * blockDim.x +
      threadIdx.x;
...
__global__ void
AdEllpackVectorProductCuda2(const AdEllpack<Real, Devices::Cuda, Index> *matrix,
                            const InVector *inVector, OutVector *outVector,
                            const int gridIdx) {
...
template <typename Real, typename Index, typename InVector, typename OutVector>
__global__ void
AdEllpackVectorProductCuda4(const AdEllpack<Real, Devices::Cuda, Index> *matrix,
                            const InVector *inVector, OutVector *outVector,
                            const int gridIdx) {
...
template <typename Real, typename Index, typename InVector, typename OutVector>
__global__ void
AdEllpackVectorProductCuda8(const AdEllpack<Real, Devices::Cuda, Index> *matrix,
                            const InVector *inVector, OutVector *outVector,
                            const int gridIdx) {
...
template <typename Real, typename Index, typename InVector, typename OutVector>
__global__ void AdEllpackVectorProductCuda16(
    const AdEllpack<Real, Devices::Cuda, Index> *matrix,
    const InVector *inVector, OutVector *outVector, const int gridIdx) {
  matrix->spmvCuda16(*inVector, *outVector, gridIdx);
}

template <typename Real, typename Index, typename InVector, typename OutVector>
__global__ void AdEllpackVectorProductCuda32(
    const AdEllpack<Real, Devices::Cuda, Index> *matrix,
    const InVector *inVector, OutVector *outVector, const int gridIdx) {
...
template <> class AdEllpackDeviceDependentCode<Devices::Cuda> {
public:
  ...
  static void vectorProduct(const AdEllpack<Real, Device, Index> &matrix,
    ...
    if (matrix.totalLoad < 2) {
      ...
      IndexType cudaBlocks =
          roundUpDivision(matrix.reduceMap.getSize(), blockSize.x);
      IndexType cudaGrids =
