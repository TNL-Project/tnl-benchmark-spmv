...
__cuda_callable__ Index BiEllpack<Real, Device, Index>::power(
    const IndexType number, const IndexType exponent) const {
  if (exponent >= 0) {
    IndexType result = 1;
...
template <typename Real, typename Device, typename Index>
BiEllpack<Real, Device, Index>::BiEllpack() : warpSize(32), logWarpSize(5) {}

template <typename Real, typename Device, typename Index>
void BiEllpack<Real, Device, Index>::setDimensions(const IndexType rows,
                                                   const IndexType columns) {
  TNL_ASSERT_GE(rows, 0, "");
  TNL_ASSERT_GE(columns, 0, "");

  if (this->getRows() % this->warpSize != 0)
    this->setVirtualRows(this->getRows() + this->warpSize -
                         (this->getRows() % this->warpSize));
  else
...
template <typename Real, typename Device, typename Index>
void BiEllpack<Real, Device, Index>::setCompressedRowLengths(
    ConstRowCapacitiesTypeView constRowLengths) {
  ...
  return this->allocateMatrixElements(
      this->warpSize *
      this->groupPointers.getElement(strips * (this->logWarpSize + 1)));
}

template <typename Real, typename Device, typename Index>
void BiEllpack<Real, Device, Index>::setRowCapacities(
    ConstRowCapacitiesTypeView constRowLengths) {
  setCompressedRowLengths(constRowLengths);
}

template <typename Real, typename Device, typename Index>
void BiEllpack<Real, Device, Index>::getCompressedRowLengths(
    RowCapacitiesTypeView rowLengths) const {
...
BiEllpack<Real, Device, Index>::getStripLength(const IndexType strip) const {
...
template <typename Real, typename Device, typename Index>
__cuda_callable__ Index
BiEllpack<Real, Device, Index>::getNumberOfGroups(const IndexType row) const {
  TNL_ASSERT_GE(row, 0, "");
  TNL_ASSERT_LT(row, this->getRows(), "");

  IndexType strip = row / this->warpSize;
  IndexType rowStripPermutation =
      this->rowPermArray.getElement(row) - this->warpSize * strip;
...
Index BiEllpack<Real, Device, Index>::getRowLength(const IndexType row) const {
  ...
  const IndexType rowStripPerm =
      this->rowPermArray.getElement(row) - strip * this->warpSize;
  IndexType elementPtr =
      this->groupPointers.getElement(groupBegin) * this->warpSize +
      rowStripPerm;
  ...
  for (IndexType group = 0; group < this->getNumberOfGroups(row); group++) {
    for (IndexType i = 0;
         i < rowMultiplicator * this->getGroupLength(strip, group); i++) {
...
void BiEllpack<Real, Device, Index>::setLike(
    const BiEllpack<Real2, Device2, Index2> &matrix) {
...
template <typename Real, typename Device, typename Index>
template <typename Real2, typename Device2, typename Index2>
bool BiEllpack<Real, Device, Index>::operator==(
    const BiEllpack<Real2, Device2, Index2> &matrix) const {
...
template <typename Real, typename Device, typename Index>
void BiEllpack<Real, Device, Index>::getRowLengths(
    RowCapacitiesType &rowLengths) const {
  // WHAT IS THIS??!
  // It's called getRowLengths, but takes an argument that it fill up with this
  // matrix's row lengths???
  for (IndexType row = 0; row < this->getRows(); row++)
    rowLengths.setElement(row, this->getRowLength(row));
}

template <typename Real, typename Device, typename Index>
bool BiEllpack<Real, Device, Index>::setElement(const IndexType row,
...
template <typename Real, typename Device, typename Index>
__cuda_callable__ bool BiEllpack<Real, Device, Index>::setElementFast(
    const IndexType row, const IndexType column, const RealType &value) {
  TNL_ASSERT_GE(row, 0, "");
  TNL_ASSERT_LT(row, this->getRows(), "");
  TNL_ASSERT_GE(column, 0, "");
  TNL_ASSERT_LT(column, this->getColumns(), "");
...
template <typename Real, typename Device, typename Index>
bool BiEllpack<Real, Device, Index>::addElement(
    const IndexType row, const IndexType column, const RealType &value,
  ...
  for (IndexType group = 0; group < this->getNumberOfGroups(row); group++) {
    for (IndexType i = 0;
         i < rowMultiplicator * this->getGroupLength(strip, group); i++) {
      if (this->columnIndexes.getElement(elementPtr) ==
      ...
      if (this->columnIndexes.getElement(elementPtr) == column) {
        this->values.setElement(
            elementPtr, value + thisElementMultiplicator *
...
template <typename Real, typename Device, typename Index>
__cuda_callable__ bool BiEllpack<Real, Device, Index>::addElementFast(
    const IndexType row, const IndexType column, const RealType &value,
    const RealType &thisElementMultiplicator) {
  const IndexType strip = row / this->warpSize;
  const IndexType groupBegin = strip * (this->logWarpSize + 1);
  const IndexType rowStripPerm =
      this->rowPermArray[row] - strip * this->warpSize;
  IndexType elementPtr =
      this->groupPointers[groupBegin] * this->warpSize + rowStripPerm;
  IndexType rowMultiplicator = 1;
  IndexType step = this->warpSize;
  ...
  for (IndexType group = 0; group < numberOfGroups; group++) {
    IndexType groupLength =
        this->groupPointers[strip * (this->logWarpSize + 1) + group + 1] -
    ...
    for (IndexType i = 0; i < rowMultiplicator * groupLength; i++) {
      ...
      if (this->columnIndexes[elementPtr] == column) {
        this->values[elementPtr] =
            thisElementMultiplicator * this->values[elementPtr] + value;
        return true;
      }
      elementPtr += step;
    }
...
template <typename Real, typename Device, typename Index>
bool BiEllpack<Real, Device, Index>::setRow(const IndexType row,
                                            const IndexType *columns,
  ...
  for (IndexType group = 0; (group < this->getNumberOfGroups(row)) &&
                            (thisElementPtr < numberOfElements);
       group++) {
    for (IndexType i = 0;
         (i < rowMultiplicator * this->getGroupLength(strip, group)) &&
         (thisElementPtr < numberOfElements);
         i++) {
...
template <typename Real, typename Device, typename Index>
bool BiEllpack<Real, Device, Index>::addRow(
    const IndexType row, const IndexType *columns, const RealType *values,
    const IndexType numberOfElements,
    const RealType &thisElementMultiplicator) {
  TNL_ASSERT_GE(row, 0, "");
  TNL_ASSERT_LT(row, this->getRows(), "");
  ...
  while (thisElementPtr < numberOfElements) {
    for (IndexType group = 0; group < this->getNumberOfGroups(row); group++) {
      for (IndexType i = 0;
           ...
           i++) {
        if (this->columnIndexes.getElement(elementPtr) ==
            columns[thisElementPtr]) {
          RealType result = this->values.getElement(elementPtr) +
                            values[thisElementPtr] * thisElementMultiplicator;
...
template <typename Real, typename Device, typename Index>
__cuda_callable__ Real BiEllpack<Real, Device, Index>::getElementFast(
    const IndexType row, const IndexType column) const {
  const IndexType strip = row / this->warpSize;
  const IndexType groupBegin = strip * (this->logWarpSize + 1);
  const IndexType rowStripPerm =
      this->rowPermArray[row] - strip * this->warpSize;
...
void BiEllpack<Real, Device, Index>::getRow(const IndexType row,
  ...
  for (IndexType group = 0; group < this->getNumberOfGroups(row) && !padding;
       group++) {
    for (IndexType i = 0;
         (i < rowMultiplicator * this->getGroupLength(strip, group)) &&
         !padding;
...
template <typename Real, typename Device, typename Index>
__cuda_callable__ Index BiEllpack<Real, Device, Index>::getGroupLength(
    const Index strip, const Index group) const {
  return this->groupPointers.getElement(strip * (this->logWarpSize + 1) +
                                        group + 1) -
         this->groupPointers.getElement(strip * (this->logWarpSize + 1) +
                                        group);
}
...
template <typename Real, typename Device, typename Index>
template <typename InVector, typename OutVector>
void BiEllpack<Real, Device, Index>::vectorProductHost(
    const InVector &inVector, OutVector &outVector) const {
  ...
  for (IndexType blockIdx = 0; blockIdx < cudaBlocks; blockIdx++) {
    ...
    for (IndexType threadIdx = 0; threadIdx < cudaBlockSize; threadIdx++) {
      ...
      for (IndexType group = 0; group < this->logWarpSize + 1; group++) {
        ...
        for (IndexType i = 0; i < groupLength; i++) {
          ...
          RealType result =
              tempStripOutVector.getElement(currentRow % cudaBlockSize);
          result += inVector[this->columnIndexes.getElement(elementPtr)] *
                    this->values.getElement(elementPtr);
    ...
    for (IndexType i = cudaBlockSize * blockIdx; i < end; i++)
      outVector[i] = tempStripOutVector.getElement(
          this->rowPermArray.getElement(i) % cudaBlockSize);
...
BiEllpack<Real, Device, Index> &BiEllpack<Real, Device, Index>::operator=(
    const BiEllpack<Real2, Device2, Index2> &matrix) {
  static_assert(std::is_same<Device, Devices::Host>::value ||
                    std::is_same<Device, Devices::Cuda>::value,
                "unknown device");
  static_assert(std::is_same<Device2, Devices::Host>::value ||
                    std::is_same<Device2, Devices::Cuda>::value,
                "unknown device");
  ...
  for (Index i = 0; i < this->values.getSize(); i++) {
    if (this->columnIndexes.getElement(i) != this->getColumns())
      std::cout << "values.getElement( " << i
                << " ) = " << this->values.getElement(i)
                << "\tcolumnIndexes.getElement( " << i
                << " ) = " << this->columnIndexes.getElement(i) << std::endl;
  }

  for (Index i = 0; i < this->rowPermArray.getSize(); i++) {
    std::cout << "rowPermArray[ " << i
              << " ] = " << this->rowPermArray.getElement(i) << std::endl;
  }

  for (Index i = 0; i < this->groupPointers.getSize(); i++) {
    std::cout << "groupPointers[ " << i
              << " ] = " << this->groupPointers.getElement(i) << std::endl;
  }
}

template <typename Real, typename Device, typename Index>
void BiEllpack<Real, Device, Index>::performRowBubbleSort(
    ...
    while (!sorted) {
      ...
      for (Index j = begin + offset; j < end - offset; j++)
        if (tempRowLengths.getElement(j) < tempRowLengths.getElement(j + 1)) {
          ...
          this->rowPermArray.setElement(
              permIndex1, this->rowPermArray.getElement(permIndex2));
...
template <typename Real, typename Device, typename Index>
void BiEllpack<Real, Device, Index>::computeColumnSizes(
    Containers::Vector<Index, Device, Index> &tempRowLengths) {
  ...
  for (Index strip = 0; strip < numberOfStrips; strip++) {
    ...
    i += emptyGroups;
    for (Index group = groupBegin + emptyGroups;
         group < groupBegin + this->logWarpSize; group++) {
      ...
      for (Index prevGroups = groupBegin; prevGroups < group; prevGroups++)
        temp -= this->power(2, prevGroups - groupBegin) *
                this->groupPointers.getElement(prevGroups);
      temp = ceil((float)temp / this->power(2, i));
      this->groupPointers.setElement(group, temp);
      i++;
    }
    Index temp = tempRowLengths.getElement(rowBegin);
    for (Index prevGroups = groupBegin;
         prevGroups < groupBegin + this->logWarpSize; prevGroups++)
      temp -= this->power(2, prevGroups - groupBegin) *
              this->groupPointers.getElement(prevGroups);
...
template <> class BiEllpackDeviceDependentCode<Devices::Host> {
public:
  typedef Devices::Host Device;

  template <typename Real, typename Index>
  static void verifyRowLengths(
      const BiEllpack<Real, Device, Index> &matrix,
      const typename BiEllpack<Real, Device, Index>::RowCapacitiesType
          &rowLengths) {
    bool ok = true;
    for (Index row = 0; row < matrix.getRows(); row++) {
      ...
      const Index rowStripPerm =
          matrix.rowPermArray.getElement(row) - strip * matrix.warpSize;
      const Index begin =
          matrix.groupPointers.getElement(groupBegin) * matrix.warpSize +
          rowStripPerm * stripLength;
      ...
      for (Index group = 0; group < matrix.getNumberOfGroups(row); group++) {
        for (Index i = 0; i < matrix.getGroupLength(strip, group); i++) {
          ...
          for (Index j = 0; j < matrix.power(2, group); j++) {
            rowLength++;
            biElementPtr +=
                matrix.power(2, matrix.logWarpSize - group) * stripLength;
          }
          elementPtr++;
        }
      }
  ...
  template <typename Real, typename Index>
  static void
  verifyRowPerm(const BiEllpack<Real, Device, Index> &matrix,
                const typename BiEllpack<Real, Device, Index>::RowCapacitiesType
                    &rowLengths) {
    bool ok = true;
    ...
    for (Index strip = 0; strip < numberOfStrips; strip++) {
      ...
      for (Index i = begin; i < end - 1; i++) {
        ...
        if (rowLengths.getElement(permIndex1) >=
            rowLengths.getElement(permIndex2))
          continue;
        else
          ok = false;
      }
  ...
  template <typename Real, typename Index, typename InVector,
            typename OutVector>
  static void vectorProduct(const BiEllpack<Real, Device, Index> &matrix,
                            const InVector &inVector, OutVector &outVector) {
    matrix.vectorProductHost(inVector, outVector);
  }

  template <typename Real, typename Index>
  static void computeColumnSizes(
      BiEllpack<Real, Device, Index> &matrix,
      const typename BiEllpack<Real, Device, Index>::RowCapacitiesType
          &rowLengths) {
    Index numberOfStrips = matrix.virtualRows / matrix.warpSize;
    for (Index strip = 0; strip < numberOfStrips; strip++) {
      ...
      if (strip == numberOfStrips - 1) {
        ...
        while (
            !(lastRows > matrix.power(2, matrix.logWarpSize - 1 - emptyGroups)))
          emptyGroups++;
        for (Index group = groupBegin; group < groupBegin + emptyGroups;
             group++)
          matrix.groupPointers.setElement(group, 0);
      }
      i += emptyGroups;
      for (Index group = groupBegin + emptyGroups;
           group < groupBegin + matrix.logWarpSize; group++) {
        ...
        for (Index prevGroups = groupBegin; prevGroups < group; prevGroups++)
          temp -= matrix.power(2, prevGroups - groupBegin) *
                  matrix.groupPointers.getElement(prevGroups);
      ...
      for (Index prevGroups = groupBegin;
           prevGroups < groupBegin + matrix.logWarpSize; prevGroups++)
        temp -= matrix.power(2, prevGroups - groupBegin) *
                matrix.groupPointers.getElement(prevGroups);
  ...
  template <typename Real, typename Index>
  static void performRowBubbleSort(
      BiEllpack<Real, Device, Index> &matrix,
      const typename BiEllpack<Real, Device, Index>::RowCapacitiesType
          &rowLengths
      /*Containers::Vector< Index, Device, Index >& tempRowLengths*/) {
      ...
      while (!sorted) {
        ...
        for (Index j = begin + offset; j < end - offset; j++) {
          ...
          if (rowLengths.getElement(permIndex1) <
            ...
            matrix.rowPermArray.setElement(
                permIndex1, matrix.rowPermArray.getElement(permIndex2));
        ...
        for (Index j = end - 1 - offset; j > begin + offset; j--) {
          ...
          if (rowLengths.getElement(permIndex2) <
              rowLengths.getElement(permIndex1)) {
            Index temp = matrix.rowPermArray.getElement(permIndex1);
            matrix.rowPermArray.setElement(
                permIndex1, matrix.rowPermArray.getElement(permIndex2));
            matrix.rowPermArray.setElement(permIndex2, temp);
...
template <typename Real, typename Device, typename Index>
template <typename InVector, typename OutVector>
__device__ void BiEllpack<Real, Device, Index>::spmvCuda(
    const InVector &inVector, OutVector &outVector, int globalIdx) const {
  ...
  IndexType elementPtr =
      (this->groupPointers[groupBegin] << this->logWarpSize) + inWarpIdx;
    ...
    if (groupLength > 0) {
      for (IndexType i = 0; i < groupLength; i++) {
        if (this->columnIndexes[elementPtr] < this->getColumns())
          temp[threadIdx.x] += inVector[this->columnIndexes[elementPtr]] *
                               this->values[elementPtr];
        elementPtr += this->warpSize;
      }
  ...
  outVector[warpStart + inWarpIdx] =
      results[this->rowPermArray[warpStart + inWarpIdx] & (cudaBlockSize - 1)];
}
#endif
...
__global__ void
BiEllpackVectorProductCuda(const BiEllpack<Real, Devices::Cuda, Index> *matrix,
                           const InVector *inVector, OutVector *outVector,
                           int gridIdx, const int warpSize) {
  Index globalIdx =
      (gridIdx * Backend::getMaxGridXSize() + blockIdx.x) * blockDim.x +
      threadIdx.x;
...
BiEllpack<Real, Device, Index>::performRowBubbleSortCudaKernel(
    const typename BiEllpack<Real, Device, Index>::RowCapacitiesType
        &rowLengths,
...
template <typename Real, typename Device, typename Index>
__cuda_callable__ void
BiEllpack<Real, Device, Index>::computeColumnSizesCudaKernel(
    const typename BiEllpack<Real, Device, Index>::RowCapacitiesType
        &rowLengths,
    const IndexType numberOfStrips, const IndexType strip) {
  ...
  if (strip == numberOfStrips - 1) {
    ...
    while (!(lastRows > this->power(2, this->logWarpSize - 1 - emptyGroups)))
      emptyGroups++;
    for (IndexType group = groupBegin; group < groupBegin + emptyGroups;
         group++)
      this->groupPointers[group] = 0;
  }
  i += emptyGroups;
  for (IndexType group = groupBegin + emptyGroups;
       group < groupBegin + this->logWarpSize; group++) {
    IndexType row = this->power(2, 4 - i);
    IndexType permRow = 0;
    while (this->rowPermArray[permRow + rowBegin] != row + rowBegin &&
           permRow < this->warpSize)
      permRow++;
    IndexType temp = rowLengths[permRow + rowBegin];
    for (IndexType prevGroups = groupBegin; prevGroups < group; prevGroups++)
      temp -= this->power(2, prevGroups - groupBegin) *
              this->groupPointers[prevGroups];
    temp = ceil((float)temp / this->power(2, i));
    this->groupPointers[group] = temp;
    i++;
  }
  IndexType permRow = rowBegin;
  while (this->rowPermArray[permRow] != rowBegin &&
         permRow < this->warpSize + rowBegin)
    permRow++;
  IndexType temp = rowLengths[permRow];
  for (IndexType prevGroups = groupBegin;
       prevGroups < groupBegin + this->logWarpSize; prevGroups++)
    temp -= this->power(2, prevGroups - groupBegin) *
            this->groupPointers[prevGroups];
  temp = ceil((float)temp / this->power(2, this->logWarpSize));
...
#ifdef __CUDACC__
template <typename Real, typename Index>
__global__ void performRowBubbleSortCuda(
    BiEllpack<Real, Devices::Cuda, Index> *matrix,
    const typename BiEllpack<Real, Devices::Cuda, Index>::RowCapacitiesType
        *rowLengths,
    int gridIdx) {
  const Index stripIdx = gridIdx * Backend::getMaxGridXSize() * blockDim.x +
...
#ifdef __CUDACC__
template <typename Real, typename Index>
__global__ void computeColumnSizesCuda(
    BiEllpack<Real, Devices::Cuda, Index> *matrix,
    const typename BiEllpack<Real, Devices::Cuda, Index>::RowCapacitiesType
        *rowLengths,
    const Index numberOfStrips, int gridIdx) {
  const Index stripIdx = gridIdx * Backend::getMaxGridXSize() * blockDim.x +
                         blockIdx.x * blockDim.x + threadIdx.x;
...
template <> class BiEllpackDeviceDependentCode<Devices::Cuda> {
public:
  ...
  static void performRowBubbleSort(
    ...
    const Index cudaGrids =
        roundUpDivision(cudaBlocks, Backend::getMaxGridXSize());
    for (int gridIdx = 0; gridIdx < cudaGrids; gridIdx++) {
      ...
      // clang-format off
         performRowBubbleSortCuda< Real, Index >
            <<< cudaGridSize, cudaBlockSize >>>( kernel_this, kernel_rowLengths, gridIdx );
      // clang-format on
    }
    ...
    for (int gridIdx = 0; gridIdx < cudaGrids; gridIdx++) {
      ...
      // clang-format off
         computeColumnSizesCuda< Real, Index >
            <<< cudaGridSize, cudaBlockSize >>>( kernel_this, kernel_rowLengths, numberOfStrips, gridIdx );
      // clang-format on
    }
    Cuda::freeFromDevice(kernel_this);
    Cuda::freeFromDevice(kernel_rowLengths);
  ...
  static void vectorProduct(const BiEllpack<Real, Device, Index> &matrix,
    ...
    const IndexType cudaBlocks =
        roundUpDivision(matrix.getRows(), cudaBlockSize.x);
    ...
    for (IndexType gridIdx = 0; gridIdx < cudaGrids; gridIdx++) {
      ...
      // clang-format off
         BiEllpackVectorProductCuda< Real, Index, InVector, OutVector ><<< cudaGridSize, cudaBlockSize, sharedMemory >>>(
            kernel_this, kernel_inVector, kernel_outVector, gridIdx, matrix.warpSize );
      // clang-format on
    }
    Cuda::freeFromDevice(kernel_this);
