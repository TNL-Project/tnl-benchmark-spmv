...
union Block
{
   ...
   Block&
   operator=( const Block< Index2 >& source )
   {
...
enum CSRKernel
{
   CSRScalar,
   CSRVector,
   CSRHybrid,  // Hybrid is not implemented
   CSRLight,
   CSRLight2,
   CSRLight3,
   CSRLight4,
...
public:
   ...
   template< typename Real2, typename Device2, typename Index2, CSRKernel KernelType2 >
   void
   setLike( const CSR< Real2, Device2, Index2, KernelType2 >& matrix );
   ...
   void
   addMatrix( const CSR< Real2, Device, Index2, KernelType2 >& matrix,
              const RealType& matrixMultiplicator = 1.0,
              const RealType& thisMatrixMultiplicator = 1.0 );

   template< typename Real2, typename Index2, CSRKernel KernelType2 >
   void
   getTransposition( const CSR< Real2, Device, Index2, KernelType2 >& matrix, const RealType& matrixMultiplicator = 1.0 );

   // copy assignment
   CSR&
   operator=( const CSR& matrix );

   template< typename IndexType2, CSRKernel KernelType2 >
   CSR&
   operator=( const CSR< RealType, DeviceType, IndexType2, KernelType2 >& matrix );
   ...
   CSR&
   operator=( const CSR< Real2, Device2, Index2, KernelType2 >& matrix );
   ...
   void
   setCudaWarpSize( const int warpSize );

   int
   getCudaWarpSize() const;

   void
   setHybridModeSplit( const IndexType hybridModeSplit );

   __cuda_callable__
   IndexType
   getHybridModeSplit() const;
