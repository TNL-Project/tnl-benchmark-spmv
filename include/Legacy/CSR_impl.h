...
template< typename Real, typename Device, typename Index, CSRKernel KernelType >
...
template< typename Real, typename Device, typename Index, CSRKernel KernelType >
std::string
CSR< Real, Device, Index, KernelType >::getSerializationType()
{
   return "Matrices::CSR< " + TNL::getType< Real >() + ", [any_device], " + TNL::getType< Index >() + " >";
}

template< typename Real, typename Device, typename Index, CSRKernel KernelType >
std::string
CSR< Real, Device, Index, KernelType >::getSerializationTypeVirtual() const
{
...
template< typename Real, typename Device, typename Index, CSRKernel KernelType >
void
CSR< Real, Device, Index, KernelType >::setDimensions( const IndexType rows, const IndexType columns )
{
...
template< typename Real, typename Device, typename Index, CSRKernel KernelType >
void
CSR< Real, Device, Index, KernelType >::setCompressedRowLengths( ConstRowCapacitiesTypeView rowLengths )
{
...
template< typename Real, typename Device, typename Index, CSRKernel KernelType >
void
CSR< Real, Device, Index, KernelType >::setRowCapacities( ConstRowCapacitiesTypeView rowLengths )
{
   setCompressedRowLengths( rowLengths );
}

/* Find limit of block */
template< typename Real, typename Index, typename Device, CSRKernel KernelType >
Index
findLimit( const Index start, const CSR< Real, Device, Index, KernelType >& matrix, const Index size, Type& type, Index& sum )
{
   sum = 0;
   for( Index current = start; current < size - 1; ++current ) {
      Index elements = matrix.getRowPointers().getElement( current + 1 ) - matrix.getRowPointers().getElement( current );
      sum += elements;
...
template< typename Real, typename Device, typename Index, CSRKernel KernelType >
void
CSR< Real, Device, Index, KernelType >::setBlocks()
{
   ...
   while( nextStart != rows - 1 ) {
      Type type;
      nextStart = findLimit< Real, Index, Device, KernelType >( start, *this, rows, type, sum );
      if( type == Type::LONG ) {
         ...
         inBlock.emplace_back(
            start, type, nextStart, this->rowPointers.getElement( nextStart ), this->rowPointers.getElement( start ) );
      }
      start = nextStart;
   }
...
template< typename Real, typename Device, typename Index, CSRKernel KernelType >
void
CSR< Real, Device, Index, KernelType >::getCompressedRowLengths( RowCapacitiesTypeView rowLengths ) const
{
...
template< typename Real, typename Device, typename Index, CSRKernel KernelType >
Index
CSR< Real, Device, Index, KernelType >::getRowLength( const IndexType row ) const
{
...
__cuda_callable__
Index
CSR< Real, Device, Index, KernelType >::getRowLengthFast( const IndexType row ) const
{
...
template< typename Real, typename Device, typename Index, CSRKernel KernelType >
Index
CSR< Real, Device, Index, KernelType >::getNonZeroRowLength( const IndexType row ) const
{
   // TODO: Fix/Implement
...
__cuda_callable__
Index
CSR< Real, Device, Index, KernelType >::getNonZeroRowLengthFast( const IndexType row ) const
{
...
void
CSR< Real, Device, Index, KernelType >::setLike( const CSR< Real2, Device2, Index2, KernelType2 >& matrix )
{
...
template< typename Real, typename Device, typename Index, CSRKernel KernelType >
void
CSR< Real, Device, Index, KernelType >::reset()
{
...
__cuda_callable__
bool
CSR< Real, Device, Index, KernelType >::setElementFast( const IndexType row, const IndexType column, const Real& value )
{
...
template< typename Real, typename Device, typename Index, CSRKernel KernelType >
bool
CSR< Real, Device, Index, KernelType >::setElement( const IndexType row, const IndexType column, const Real& value )
{
...
__cuda_callable__
bool
CSR< Real, Device, Index, KernelType >::addElementFast( const IndexType row,
                                                        ...
                                                        const RealType& thisElementMultiplicator )
{
   ...
   IndexType col = 0;
   while( elementPtr < rowEnd && ( col = this->columnIndexes[ elementPtr ] ) < column && col != this->getPaddingIndex() )
      elementPtr++;
   ...
   else if( col == this->getPaddingIndex() ) {
...
template< typename Real, typename Device, typename Index, CSRKernel KernelType >
bool
CSR< Real, Device, Index, KernelType >::addElement( const IndexType row,
                                                    const IndexType column,
                                                    const RealType& value,
                                                    const RealType& thisElementMultiplicator )
{
   ...
   IndexType col = 0;
   while( elementPtr < rowEnd && ( col = this->columnIndexes.getElement( elementPtr ) ) < column
...
__cuda_callable__
bool
CSR< Real, Device, Index, KernelType >::setRowFast( const IndexType row,
...
template< typename Real, typename Device, typename Index, CSRKernel KernelType >
bool
CSR< Real, Device, Index, KernelType >::setRow( const IndexType row,
                                                const IndexType* columnIndexes,
                                                const RealType* values,
                                                const IndexType elements )
{
...
__cuda_callable__
bool
CSR< Real, Device, Index, KernelType >::addRowFast( const IndexType row,
...
template< typename Real, typename Device, typename Index, CSRKernel KernelType >
bool
CSR< Real, Device, Index, KernelType >::addRow( const IndexType row,
                                                const IndexType* columns,
                                                const RealType* values,
                                                const IndexType numberOfElements,
                                                const RealType& thisElementMultiplicator )
...
__cuda_callable__
Real
CSR< Real, Device, Index, KernelType >::getElementFast( const IndexType row, const IndexType column ) const
{
   IndexType elementPtr = this->rowPointers[ row ];
   const IndexType rowEnd = this->rowPointers[ row + 1 ];
   IndexType col = 0;
...
template< typename Real, typename Device, typename Index, CSRKernel KernelType >
Real
CSR< Real, Device, Index, KernelType >::getElement( const IndexType row, const IndexType column ) const
{
   IndexType elementPtr = this->rowPointers.getElement( row );
   const IndexType rowEnd = this->rowPointers.getElement( row + 1 );
   IndexType col = 0;
...
__cuda_callable__
void
CSR< Real, Device, Index, KernelType >::getRowFast( const IndexType row, IndexType* columns, RealType* values ) const
{
   IndexType elementPointer = this->rowPointers[ row ];
   const IndexType rowLength = this->rowPointers[ row + 1 ] - elementPointer;
...
CSR< Real, Device, Index, KernelType >::getRow( const IndexType rowIndex )
{
...
CSR< Real, Device, Index, KernelType >::getRow( const IndexType rowIndex ) const
{
   const IndexType rowOffset = this->rowPointers[ rowIndex ];
   const IndexType rowLength = this->rowPointers[ rowIndex + 1 ] - rowOffset;
...
CSR< Real, Device, Index, KernelType >::rowVectorProduct( const IndexType row, const Vector& vector ) const
{
   Real result = 0.0;
   IndexType elementPtr = this->rowPointers[ row ];
   const IndexType rowEnd = this->rowPointers[ row + 1 ];
...
void
CSR< Real, Device, Index, KernelType >::vectorProduct( const InVector& inVector, OutVector& outVector ) const
{
...
void
CSR< Real, Device, Index, KernelType >::addMatrix( const CSR< Real2, Device, Index2, KernelType2 >& matrix,
...
template< typename Real2, typename Index2, CSRKernel KernelType2 >
void
CSR< Real, Device, Index, KernelType >::getTransposition( const CSR< Real2, Device, Index2, KernelType2 >& matrix,
                                                          const RealType& matrixMultiplicator )
{
...
CSR< Real, Device, Index, KernelType >&
CSR< Real, Device, Index, KernelType >::operator=( const CSR< Real, Device, IndexType2, KernelType2 >& matrix )
{
   this->setLike( matrix );
   this->values = matrix.values;
   this->columnIndexes = matrix.columnIndexes;
...
template< typename Real, typename Device, typename Index, CSRKernel KernelType >
void
CSR< Real, Device, Index, KernelType >::save( File& file ) const
{
...
template< typename Real, typename Device, typename Index, CSRKernel KernelType >
void
CSR< Real, Device, Index, KernelType >::load( File& file )
{
...
template< typename Real, typename Device, typename Index, CSRKernel KernelType >
void
CSR< Real, Device, Index, KernelType >::save( const String& fileName ) const
{}

template< typename Real, typename Device, typename Index, CSRKernel KernelType >
void
CSR< Real, Device, Index, KernelType >::load( const String& fileName )
{}

template< typename Real, typename Device, typename Index, CSRKernel KernelType >
void
CSR< Real, Device, Index, KernelType >::print( std::ostream& str ) const
{
   for( IndexType row = 0; row < this->getRows(); row++ ) {
      str << "Row: " << row << " -> ";
      IndexType elementPtr = this->rowPointers.getElement( row );
      const IndexType rowEnd = this->rowPointers.getElement( row + 1 );
      IndexType column;
      while( elementPtr < rowEnd && ( column = this->columnIndexes.getElement( elementPtr ) ) < this->columns
...
template< typename Real,
          ...
          typename Index >
__cuda_callable__
typename CSR< Real, Device, Index, KernelType >::SPMVCudaKernel CSR< Real, Device, Index, KernelType >::getCudaKernelType()
const
{
...
template< typename Real, typename Device, typename Index, CSRKernel KernelType >
void
CSR< Real, Device, Index, KernelType >::setCudaWarpSize( const int warpSize )
{
...
template< typename Real, typename Device, typename Index, CSRKernel KernelType >
int
CSR< Real, Device, Index, KernelType >::getCudaWarpSize() const
{
...
template< typename Real, typename Device, typename Index, CSRKernel KernelType >
void
CSR< Real, Device, Index, KernelType >::setHybridModeSplit( const IndexType hybridModeSplit )
{
...
__cuda_callable__
Index
CSR< Real, Device, Index, KernelType >::getHybridModeSplit() const
{
...
__global__
void
SpMVCSRScalar( const Real* inVector,
               Real* outVector,
...
template< typename Real, typename Index, int warpSize >
__global__
void
SpMVCSRMultiVector( const Real* inVector,
                    Real* outVector,
                    const Index* rowPointers,
                    const Index* columnIndexes,
                    const Real* values,
                    ...
                    const Index gridID )
{
   ...
   /* Calculate result */
   for( Index i = rowPointers[ rowID ] + ( warpID % warps ) * warpSize + laneID; i < endID; i += offset ) {
      result += values[ i ] * inVector[ columnIndexes[ i ] ];
   }
   ...
   /* Write result */
   if( laneID == 0 )
      atomicAdd( &outVector[ rowID ], result );
}

template< typename Real, typename Index, int warpSize >
__global__
void
SpMVCSRVector( const Real* inVector,
               Real* outVector,
               const Index* rowPointers,
               const Index* columnIndexes,
               const Real* values,
               ...
               const Index gridID )
{
   ...
   /* Write result */
   if( laneID == 0 )
      outVector[ warpID ] = result;
}

template< typename Real, typename Index, int groupSize, int MAX_NUM_VECTORS_PER_BLOCK >
__global__
void
SpMVCSRLight( const Real* inVector,
              Real* outVector,
              const Index* rowPointers,
              const Index* columnIndexes,
              const Real* values,
...
template< typename Real, typename Index, int groupSize >
__global__
void
SpMVCSRLight2( const Real* inVector,
               Real* outVector,
               const Index* rowPointers,
               const Index* columnIndexes,
               const Real* values,
...
__global__
void
SpMVCSRLight3( const Real* inVector,
               Real* outVector,
               const Index* rowPointers,
               const Index* columnIndexes,
               const Real* values,
...
__global__
void
SpMVCSRLight4( const Real* inVector,
               Real* outVector,
               const Index* rowPointers,
               const Index* columnIndexes,
               const Real* values,
...
__global__
void
SpMVCSRLightWithoutAtomic2( const Real* inVector,
                            Real* outVector,
                            const Index* rowPointers,
                            const Index* columnIndexes,
                            const Real* values,
                            ...
                            const Index gridID )
{
   const Index row = ( ( gridID * MAX_X_DIM ) + ( blockIdx.x * blockDim.x ) + threadIdx.x ) / 2;
   if( row >= rows )
      return;
...
__global__
void
SpMVCSRLightWithoutAtomic4( const Real* inVector,
                            Real* outVector,
                            const Index* rowPointers,
                            const Index* columnIndexes,
                            const Real* values,
                            ...
                            const Index gridID )
{
   const Index row = ( ( gridID * MAX_X_DIM ) + ( blockIdx.x * blockDim.x ) + threadIdx.x ) / 4;
   if( row >= rows )
      return;
...
__global__
void
SpMVCSRLightWithoutAtomic8( const Real* inVector,
                            Real* outVector,
                            const Index* rowPointers,
                            const Index* columnIndexes,
                            const Real* values,
                            ...
                            const Index gridID )
{
   const Index row = ( ( gridID * MAX_X_DIM ) + ( blockIdx.x * blockDim.x ) + threadIdx.x ) / 8;
   if( row >= rows )
      return;
...
__global__
void
SpMVCSRLightWithoutAtomic16( const Real* inVector,
                             Real* outVector,
                             const Index* rowPointers,
                             const Index* columnIndexes,
                             const Real* values,
                             ...
                             const Index gridID )
{
   const Index row = ( ( gridID * MAX_X_DIM ) + ( blockIdx.x * blockDim.x ) + threadIdx.x ) / 16;
   if( row >= rows )
      return;
...
template< typename Real, typename Index, typename Device, CSRKernel KernelType >
void
SpMVCSRScalarPrepare( const Real* inVector, Real* outVector, const CSR< Real, Device, Index, KernelType >& matrix )
{
   ...
   for( Index grid = 0; neededThreads != 0; ++grid ) {
      ...
      // clang-format off
      SpMVCSRScalar< Real, Index ><<< blocks, threads >>>( inVector,
                                                               outVector,
...
template< typename Real, typename Index, typename Device, CSRKernel KernelType, int warpSize >
void
SpMVCSRVectorPrepare( const Real* inVector, Real* outVector, const CSR< Real, Device, Index, KernelType >& matrix )
{
   ...
   for( Index grid = 0; neededThreads != 0; ++grid ) {
      ...
      // clang-format off
      SpMVCSRVector< Real, Index, warpSize ><<< blocks, threads >>>( inVector,
                                                                         outVector,
                                                                         matrix.getRowPointers().getData(),
                                                                         matrix.getColumnIndexes().getData(),
                                                                         matrix.getValues().getData(),
...
template< typename Real, typename Index, typename Device, CSRKernel KernelType, int warpSize >
void
SpMVCSRLightPrepare( const Real* inVector, Real* outVector, const CSR< Real, Device, Index, KernelType >& matrix )
{
   ...
   const Index blocks = properties.multiProcessorCount * properties.maxThreadsPerMultiProcessor / threads;

   const Index nnz = roundUpDivision( matrix.getValues().getSize(), rows );  // non zeroes per row
   // clang-format off
   if( KernelType == CSRLight ) {
      if( nnz <= 2 )
         SpMVCSRLight< Real, Index, 2, 1024 / 2 ><<< blocks, threads >>>( inVector,
                                                                              outVector,
                                                                              matrix.getRowPointers().getData(),
                                                                              matrix.getColumnIndexes().getData(),
                                                                              matrix.getValues().getData(),
         ...
         SpMVCSRLight< Real, Index, 4, 1024 / 4 ><<< blocks, threads >>>( inVector,
                                                                              outVector,
                                                                              matrix.getRowPointers().getData(),
                                                                              matrix.getColumnIndexes().getData(),
                                                                              matrix.getValues().getData(),
         ...
         SpMVCSRLight< Real, Index, 8, 1024 / 8 ><<< blocks, threads >>>( inVector,
                                                                              outVector,
                                                                              matrix.getRowPointers().getData(),
                                                                              matrix.getColumnIndexes().getData(),
                                                                              matrix.getValues().getData(),
         ...
         SpMVCSRLight< Real, Index, 32, 1024 / 32 ><<< blocks, threads >>>( inVector,
                                                                                outVector,
                                                                                matrix.getRowPointers().getData(),
                                                                                matrix.getColumnIndexes().getData(),
                                                                                matrix.getValues().getData(),
      ...
      if( nnz <= 2 )
         SpMVCSRLight2< Real, Index, 2 ><<< blocks, threads >>>( inVector,
                                                                     outVector,
                                                                     matrix.getRowPointers().getData(),
                                                                     matrix.getColumnIndexes().getData(),
                                                                     matrix.getValues().getData(),
         ...
         SpMVCSRLight2< Real, Index, 4 ><<< blocks, threads >>>( inVector,
                                                                     outVector,
                                                                     matrix.getRowPointers().getData(),
                                                                     matrix.getColumnIndexes().getData(),
                                                                     matrix.getValues().getData(),
         ...
         SpMVCSRLight2< Real, Index, 8 ><<< blocks, threads >>>( inVector,
                                                                     outVector,
                                                                     matrix.getRowPointers().getData(),
                                                                     matrix.getColumnIndexes().getData(),
                                                                     matrix.getValues().getData(),
         ...
         SpMVCSRLight2< Real, Index, 32 ><<< blocks, threads >>>( inVector,
                                                                      outVector,
                                                                      matrix.getRowPointers().getData(),
                                                                      matrix.getColumnIndexes().getData(),
                                                                      matrix.getValues().getData(),
      ...
      if( nnz <= 2 )
         SpMVCSRLight3< Real, Index, 2 ><<< blocks, threads >>>( inVector,
                                                                     outVector,
                                                                     matrix.getRowPointers().getData(),
                                                                     matrix.getColumnIndexes().getData(),
                                                                     matrix.getValues().getData(),
         ...
         SpMVCSRLight3< Real, Index, 4 ><<< blocks, threads >>>( inVector,
                                                                     outVector,
                                                                     matrix.getRowPointers().getData(),
                                                                     matrix.getColumnIndexes().getData(),
                                                                     matrix.getValues().getData(),
         ...
         SpMVCSRLight3< Real, Index, 8 ><<< blocks, threads >>>( inVector,
                                                                     outVector,
                                                                     matrix.getRowPointers().getData(),
                                                                     matrix.getColumnIndexes().getData(),
                                                                     matrix.getValues().getData(),
         ...
         SpMVCSRLight3< Real, Index, 32 ><<< blocks, threads >>>( inVector,
                                                                      outVector,
                                                                      matrix.getRowPointers().getData(),
                                                                      matrix.getColumnIndexes().getData(),
                                                                      matrix.getValues().getData(),
      ...
      if( nnz <= 2 )
         ...
         SpMVCSRLight3< Real, Index, 16 ><<< blocks, threads >>>( inVector,
                                                                      outVector,
                                                                      matrix.getRowPointers().getData(),
                                                                      matrix.getColumnIndexes().getData(),
                                                                      matrix.getValues().getData(),
...
template< typename Real, typename Index, typename Device, CSRKernel KernelType, int warpSize >
void
SpMVCSRLightWithoutAtomicPrepare( const Real* inVector, Real* outVector, const CSR< Real, Device, Index, KernelType >& matrix )
{
   const Index rows = matrix.getRowPointers().getSize() - 1;
   ...
   for( Index grid = 0; neededThreads != 0; ++grid ) {
      ...
      if( KernelType == CSRLightWithoutAtomic ) {
         if( groupSize == 2 ) {
            SpMVCSRLightWithoutAtomic2< Real, Index ><<< blocks, threads >>>( inVector,
                                                                                  outVector,
                                                                                  matrix.getRowPointers().getData(),
                                                                                  matrix.getColumnIndexes().getData(),
                                                                                  matrix.getValues().getData(),
            ...
            SpMVCSRLightWithoutAtomic4< Real, Index ><<< blocks, threads >>>( inVector,
                                                                                  outVector,
                                                                                  matrix.getRowPointers().getData(),
                                                                                  matrix.getColumnIndexes().getData(),
                                                                                  matrix.getValues().getData(),
            ...
            SpMVCSRLightWithoutAtomic8< Real, Index ><<< blocks, threads >>>( inVector,
                                                                                  outVector,
                                                                                  matrix.getRowPointers().getData(),
                                                                                  matrix.getColumnIndexes().getData(),
                                                                                  matrix.getValues().getData(),
            ...
            SpMVCSRLightWithoutAtomic16< Real, Index ><<< blocks, threads >>>( inVector,
                                                                                   outVector,
                                                                                   matrix.getRowPointers().getData(),
                                                                                   matrix.getColumnIndexes().getData(),
                                                                                   matrix.getValues().getData(),
            ...
            SpMVCSRMultiVector< Real, Index, warpSize ><<< blocks, threads >>>( inVector,
                                                                                    outVector,
                                                                                    matrix.getRowPointers().getData(),
                                                                                    matrix.getColumnIndexes().getData(),
                                                                                    matrix.getValues().getData(),
         ...
         if( groupSize == 2 ) {
            SpMVCSRLight4< Real, Index, 2 ><<< blocks, threads >>>( inVector,
                                                                        outVector,
                                                                        matrix.getRowPointers().getData(),
                                                                        matrix.getColumnIndexes().getData(),
                                                                        matrix.getValues().getData(),
            ...
            SpMVCSRLight4< Real, Index, 4 ><<< blocks, threads >>>( inVector,
                                                                        outVector,
                                                                        matrix.getRowPointers().getData(),
                                                                        matrix.getColumnIndexes().getData(),
                                                                        matrix.getValues().getData(),
            ...
            SpMVCSRLight4< Real, Index, 8 ><<< blocks, threads >>>( inVector,
                                                                        outVector,
                                                                        matrix.getRowPointers().getData(),
                                                                        matrix.getColumnIndexes().getData(),
                                                                        matrix.getValues().getData(),
            ...
            SpMVCSRLight4< Real, Index, 16 ><<< blocks, threads >>>( inVector,
                                                                         outVector,
                                                                         matrix.getRowPointers().getData(),
                                                                         matrix.getColumnIndexes().getData(),
                                                                         matrix.getValues().getData(),
...
template< typename Real, typename Index, typename Device, CSRKernel KernelType, int warpSize >
void
SpMVCSRMultiVectorPrepare( const Real* inVector, Real* outVector, const CSR< Real, Device, Index, KernelType >& matrix )
{
   const Index rows = matrix.getRowPointers().getSize() - 1;
   const Index threads = matrix.THREADS_VECTOR;  // block size
   Index blocks;
...
__device__
Real
CSRFetch( const Index* columnIndexes, const Real* values, const Real* vector, const Index i )
{
   return values[ i ] * vector[ columnIndexes[ i ] ];
}

template< typename Real, typename Index, int warpSize, int WARPS, int SHARED_PER_WARP, int MAX_ELEM_PER_WARP, typename Fetch >
__global__
void
SpMVCSRAdaptive( const Real* inVector,
                 Real* outVector,
                 const Index* rowPointers,
                 const Index* columnIndexes,
                 const Real* values,
                 ...
                 const Fetch fetch )
{
   ...
   if( block.byte[ sizeof( Index ) == 4 ? 7 : 15 ] & 0b1000000 ) {
      ...
      // Write result
      if( laneID == 0 )
         outVector[ block.index[ 0 ] ] = result;  // block.index[0] -> minRow
   }
   else {
      //////////////////////////////////// CSR VECTOR L ////////////
      ...
      result += __shfl_down_sync( 0xFFFFFFFF, result, 1 );
      if( laneID == 0 )
         atomicAdd( &outVector[ block.index[ 0 ] ], result );
      //                                    ^ minRow
   }
}
...
template< typename Real, typename Index, typename Device, CSRKernel KernelType, int warpSize >
void
SpMVCSRAdaptivePrepare( const Real* inVector, Real* outVector, const CSR< Real, Device, Index, KernelType >& matrix )
{
   Index blocks;
   ...
   for( Index grid = 0; neededThreads != 0; ++grid ) {
      ...
      // clang-format off
      SpMVCSRAdaptive< Real, Index, warpSize, Matrix::WARPS, Matrix::SHARED_PER_WARP, Matrix::MAX_ELEMENTS_PER_WARP_ADAPT >
         <<< blocks, threads >>>( inVector,
                                      outVector,
                                      matrix.getRowPointers().getData(),
                                      matrix.getColumnIndexes().getData(),
                                      matrix.getValues().getData(),
...
public:
   ...
   template< typename Real, typename Index, CSRKernel KernelType, typename InVector, typename OutVector >
   static void
   vectorProduct( const CSR< Real, Device, Index, KernelType >& matrix, const InVector& inVector, OutVector& outVector )
   {
...
#ifdef HAVE_OPENMP
   #pragma omp parallel for firstprivate( matrixPtr, inVectorPtr, outVectorPtr ), schedule( dynamic, 100 ), \
...
public:
   ...
   static void
   vectorProduct( const Index rows,
                  const Index columns,
                  const Index nnz,
                  const Real* values,
                  const Index* columnIndexes,
...
public:
   ...
   vectorProduct( const CSR< Real, Device, Index, KernelType >& matrix, const InVector& inVector, OutVector& outVector )
   {
      ...
      switch( KernelType ) {
         case CSRScalar:
            SpMVCSRScalarPrepare< Real, Index, Device, KernelType >( inVector.getData(), outVector.getData(), matrix );
            break;
         case CSRVector:
            SpMVCSRVectorPrepare< Real, Index, Device, KernelType, 32 >( inVector.getData(), outVector.getData(), matrix );
            break;
         case CSRLight:
         case CSRLight2:
         case CSRLight3:
         case CSRLight6:
            SpMVCSRLightPrepare< Real, Index, Device, KernelType, 32 >( inVector.getData(), outVector.getData(), matrix );
            break;
         case CSRAdaptive:
            SpMVCSRAdaptivePrepare< Real, Index, Device, KernelType, 32 >( inVector.getData(), outVector.getData(), matrix );
            break;
         case CSRMultiVector:
            SpMVCSRMultiVectorPrepare< Real, Index, Device, KernelType, 32 >( inVector.getData(), outVector.getData(), matrix );
            break;
         case CSRLight4:
         case CSRLight5:
