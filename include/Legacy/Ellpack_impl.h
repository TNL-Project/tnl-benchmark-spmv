...
Ellpack<Real, Device, Index>::Ellpack() : rowLengths(0), alignedRows(0) {}

template <typename Real, typename Device, typename Index>
std::string Ellpack<Real, Device, Index>::getSerializationType() {
  return "Matrices::Ellpack< " + TNL::getType<Real>() + ", [any device], " +
         getType<Index>() + " >";
}
...
template <typename Real, typename Device, typename Index>
void Ellpack<Real, Device, Index>::setCompressedRowLengths(
...
template <typename Real, typename Device, typename Index>
void Ellpack<Real, Device, Index>::setRowCapacities(
    ConstRowCapacitiesTypeView rowLengths) {
  setCompressedRowLengths(rowLengths);
}

template <typename Real, typename Device, typename Index>
void Ellpack<Real, Device, Index>::getCompressedRowLengths(
...
template <typename Real, typename Device, typename Index>
void Ellpack<Real, Device, Index>::setConstantCompressedRowLengths(
    const IndexType &rowLengths) {
...
Ellpack<Real, Device, Index>::getRowLengthFast(const IndexType row) const {
  return this->rowLengths;
}

template <typename Real, typename Device, typename Index>
Index Ellpack<Real, Device, Index>::getNonZeroRowLength(
...
void Ellpack<Real, Device, Index>::setLike(
    const Ellpack<Real2, Device2, Index2> &matrix) {
  Sparse<Real, Device, Index>::setLike(matrix);
  this->rowLengths = matrix.rowLengths;
  if (std::is_same<Device, Devices::Cuda>::value)
    this->alignedRows =
        roundToMultiple(this->getRows(), Backend::getWarpSize());
  else
    this->alignedRows = this->getRows();
}
...
template <typename Real, typename Device, typename Index>
template <typename Real2, typename Device2, typename Index2>
bool Ellpack<Real, Device, Index>::operator==(
    const Ellpack<Real2, Device2, Index2> &matrix) const {
...
template <typename Real, typename Device, typename Index>
__cuda_callable__ bool Ellpack<Real, Device, Index>::setElementFast(
...
template <typename Real, typename Device, typename Index>
__cuda_callable__ bool Ellpack<Real, Device, Index>::addElementFast(
  ...
  while (i < rowEnd && this->columnIndexes[i] < column &&
         this->columnIndexes[i] != this->getPaddingIndex())
    i += step;
  if (i == rowEnd)
    return false;
  if (this->columnIndexes[i] == column) {
    ...
    return true;
  } else if (this->columnIndexes[i] ==
             this->getPaddingIndex()) // artificial zero
  {
...
template <typename Real, typename Device, typename Index>
bool Ellpack<Real, Device, Index>::addElement(
    const IndexType row, const IndexType column, const RealType &value,
    const RealType &thisElementMultiplicator) {
  typedef EllpackDeviceDependentCode<DeviceType> DDCType;
  IndexType i = DDCType::getRowBegin(*this, row);
  ...
  while (i < rowEnd && this->columnIndexes.getElement(i) < column &&
  ...
  if (this->columnIndexes.getElement(i) == column) {
    this->values.setElement(
        i, thisElementMultiplicator * this->values.getElement(i) + value);
    return true;
  } else if (this->columnIndexes.getElement(i) == this->getPaddingIndex()) {
    this->columnIndexes.setElement(i, column);
    this->values.setElement(i, value);
  } else {
    IndexType j = rowEnd - step;
    while (j > i) {
      this->columnIndexes.setElement(j,
                                     this->columnIndexes.getElement(j - step));
...
template <typename Real, typename Device, typename Index>
__cuda_callable__ bool Ellpack<Real, Device, Index>::setRowFast(
...
template <typename Real, typename Device, typename Index>
__cuda_callable__ bool Ellpack<Real, Device, Index>::addRowFast(
...
template <typename Real, typename Device, typename Index>
bool Ellpack<Real, Device, Index>::addRow(
    const IndexType row, const IndexType *columns, const RealType *values,
    const IndexType numberOfElements,
    const RealType &thisElementMultiplicator) {
...
template <typename Real, typename Device, typename Index>
__cuda_callable__ Real Ellpack<Real, Device, Index>::getElementFast(
...
template <typename Real, typename Device, typename Index>
__cuda_callable__ void Ellpack<Real, Device, Index>::getRowFast(
...
template <typename Real, typename Device, typename Index>
__cuda_callable__ typename Ellpack<Real, Device, Index>::MatrixRow
Ellpack<Real, Device, Index>::getRow(const IndexType rowIndex) {
...
template <typename Real, typename Device, typename Index>
__cuda_callable__ typename Ellpack<Real, Device, Index>::ConstMatrixRow
Ellpack<Real, Device, Index>::getRow(const IndexType rowIndex) const {
  // printf( "this->rowLengths = %d this = %p \n", this->rowLengths, this );
  IndexType rowBegin = DeviceDependentCode::getRowBegin(*this, rowIndex);
...
Ellpack<Real, Device, Index>::rowVectorProduct(const IndexType row,
...
void Ellpack<Real, Device, Index>::addMatrix(
    const Ellpack<Real2, Device, Index2> &matrix,
    const RealType &matrixMultiplicator,
    const RealType &thisMatrixMultiplicator) {
  throw Exceptions::NotImplementedError(
      "Ellpack::addMatrix is not implemented.");
  // TODO: implement
}

template <typename Real, typename Device, typename Index>
template <typename Real2, typename Index2>
void Ellpack<Real, Device, Index>::getTransposition(
    const Ellpack<Real2, Device, Index2> &matrix,
    const RealType &matrixMultiplicator) {
  throw Exceptions::NotImplementedError(
      "Ellpack::getTransposition is not implemented.");
  // TODO: implement
}
...
Ellpack<Real, Device, Index> &Ellpack<Real, Device, Index>::operator=(
    const Ellpack<Real2, Device2, Index2> &matrix) {
    ...
    for (Index b = 0; b < blocks; b++) {
      ...
      for (Index j = 0; j < rowLengths; j++)
        for (Index i = 0; i < blockSize && offset + i < this->getRows(); i++) {
          tmpValues[offset + j * alignedRows + i] =
              matrix.values[(offset + i) * rowLengths + j];
          tmpColumnIndexes[offset + j * alignedRows + i] =
              matrix.columnIndexes[(offset + i) * rowLengths + j];
        }
    }
...
#pragma omp parallel for if (Devices::Host::isOMPEnabled())
    ...
    for (Index b = 0; b < blocks; b++) {
      ...
      for (Index i = 0; i < blockSize && b * blockSize + i < this->getRows();
           i++)
        for (Index j = 0; j < rowLengths; j++) {
          this->values[offset + i * rowLengths + j] =
              tmpValues[b * blockSize + j * matrix.alignedRows + i];
          this->columnIndexes[offset + i * rowLengths + j] =
              tmpColumnIndexes[b * blockSize + j * matrix.alignedRows + i];
        }
    }
  }
...
void Ellpack<Real, Device, Index>::allocateElements() {
  ...
  if (this->alignedRows != 0) {
    TNL_ASSERT_EQ(numMtxElmnts / this->alignedRows, this->rowLengths,
                  "Ellpack cannot store this matrix. The number of matrix "
                  "elements has overflown the value that IndexType "
...
template <> class EllpackDeviceDependentCode<Devices::Host> {
public:
  typedef Devices::Host Device;

  template <typename Real, typename Index>
  __cuda_callable__ static Index
  getRowBegin(const Ellpack<Real, Device, Index> &matrix, const Index row) {
    return row * matrix.rowLengths;
  }

  template <typename Real, typename Index>
  __cuda_callable__ static Index
  getRowEnd(const Ellpack<Real, Device, Index> &matrix, const Index row) {
    return (row + 1) * matrix.rowLengths;
  }

  template <typename Real, typename Index>
  __cuda_callable__ static Index
  getElementStep(const Ellpack<Real, Device, Index> &matrix) {
    return 1;
  }
...
__global__ void EllpackVectorProductCudaKernel(
    const Index rows, const Index columns, const Index compressedRowLengths,
    const Index alignedRows, const Index paddingIndex,
...
template <> class EllpackDeviceDependentCode<Devices::Cuda> {
public:
...
// MatrixVectorProductCuda( matrix, inVector, outVector );
    ...
    for (IndexType gridIdx = 0; gridIdx < cudaGrids; gridIdx++) {
      ...
      // clang-format off
         EllpackVectorProductCudaKernel< Real, Index ><<< cudaGridSize, cudaBlockSize >>>( matrix.getRows(),
                                                                                               matrix.getColumns(),
                                                                                               matrix.rowLengths,
                                                                                               matrix.alignedRows,
