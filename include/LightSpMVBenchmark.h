...
enum LightSpMVBenchmarkKernelType
{
   LightSpMVBenchmarkKernelVector,
   LightSpMVBenchmarkKernelWarp
};
...
struct LightSpMVVectorsBinder
{
   ...
   static void
   bind( TNL::Containers::VectorView< Real1, TNL::Devices::Cuda, Index >& vectorView, Real2* data, Index size )
   {}
};
...
struct LightSpMVVectorsBinder< Real, Real >
{
   template< typename Index >
   static void
   bind( TNL::Containers::VectorView< Real, TNL::Devices::Cuda, Index >& vectorView, Real* data, Index size )
   {
...
struct LightSpMVBenchmark
{
   ...
   {
      static_assert( std::is_same< typename Matrix::DeviceType, TNL::Devices::Host >::value,
                     "The only device type accepted here is TNL::Devices::Host." );
#ifdef __CUDACC__
      cudaDeviceProp prop;
      ...
      if( std::is_same< Real, float >::value ) {
         ...
         throw std::runtime_error( "Unknown real type for LightSpMV." );
      this->spmv->loadData();
      if( std::is_same< Real, float >::value ) {
         if( kernelType == LightSpMVBenchmarkKernelVector ) {
         ...
         std::runtime_error( "Unknown real type for LightSpMV." );
#endif
   }

   void
   setKernelType( LightSpMVBenchmarkKernelType type )
   {
