...
public:
   ...
   init( MatrixType& matrix, const InVector& inVector, OutVector& outVector, cusparseHandle_t* cusparseHandle )
   {
      ...
      size_t buffer_size;
      CHECK_CUSPARSE( cusparseSpMV_bufferSize( *( this->cusparseHandle ),         // cusparseHandle_t     handle,
   ...
   vectorProduct( const InVector& inVector, OutVector& outVector ) const
   {
      ...
      double alpha = 1.0;
      double beta = 0.0;
      CHECK_CUSPARSE( cusparseSpMV( *( this->cusparseHandle ),           // cusparseHandle_t     handle,
                                    CUSPARSE_OPERATION_NON_TRANSPOSE,    // cusparseOperation_t  opA,
                                    &alpha,                              // const void*          alpha,
                                    this->matA,                          // cusparseSpMatDescr_t matA,
                                    this->vecX,                          // cusparseDnVecDescr_t vecX,
...
public:
   ...
   init( MatrixType& matrix, const InVector& inVector, OutVector& outVector, cusparseHandle_t* cusparseHandle )
   {
      ...
      CHECK_CUSPARSE( cusparseCreateDnVec( &vecX, matrix.getColumns(), (void*) inVector.getData(), CUDA_R_32F ) );
      CHECK_CUSPARSE( cusparseCreateDnVec( &vecY, matrix.getRows(), (void*) outVector.getData(), CUDA_R_32F ) );
      size_t buffer_size;
      CHECK_CUSPARSE( cusparseSpMV_bufferSize( *( this->cusparseHandle ),         // cusparseHandle_t     handle,
                                               CUSPARSE_OPERATION_NON_TRANSPOSE,  // cusparseOperation_t  opA,
                                               &alpha,                            // const void*          alpha,
